...
def main_menu():
    ...
    start_game = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            (SCREEN_WIDTH // 3, SCREEN_HEIGHT // 3), (SCREEN_WIDTH / 4, 50)
        ),
        text="START GAME",
        manager=manager,
    )
    quit_game = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            (SCREEN_WIDTH // 3, SCREEN_HEIGHT // 2), (SCREEN_WIDTH / 4, 50)
        ),
        text="QUIT GAME",
        manager=manager,
    )
...
def mode_menu():
    ...
    is_running = True
    background.blit(menu_background, (0, 0))
    option_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(
            (SCREEN_WIDTH // 3, SCREEN_HEIGHT // 4), (SCREEN_WIDTH / 4, 50)
        ),
        text="Select Board Type",
        manager=manager,
    )

    default_board = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            (SCREEN_WIDTH // 3, SCREEN_HEIGHT // 2), (SCREEN_WIDTH / 4, 50)
        ),
        text="Default Board",
    ...
    random_board = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            (SCREEN_WIDTH // 3, SCREEN_HEIGHT // 3), (SCREEN_WIDTH / 4, 50)
        ),
        text="Random Board",
...
def game_loop(mode):
    ...
    player_name_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y, 150, 150),
        text="Player " + str(new_game.current_player.name),
        manager=manager,
    )
    resources_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset - 25, 150, 150),
        text="Resources",
        manager=manager,
    )

    brick_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset, 150, 150),
        text="Brick: " + str(new_game.current_player.resources.get(TileResource.Brick)),
        manager=manager,
    )
    lumber_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 2, 150, 150),
        text="Lumber: "
        + str(new_game.current_player.resources.get(TileResource.Lumber)),
        manager=manager,
    )
    ore_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 3, 150, 150),
        text="Ore: " + str(new_game.current_player.resources.get(TileResource.Ore)),
        manager=manager,
    )
    grain_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 4, 150, 150),
        text="Grain: " + str(new_game.current_player.resources.get(TileResource.Grain)),
        manager=manager,
    )
    wool_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 5, 150, 150),
        text="Wool: " + str(new_game.current_player.resources.get(TileResource.Wool)),
        manager=manager,
    )

    victory_points_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 6, 270, 150),
        text="Victory Points: " + str(new_game.current_player.victory_points),
        manager=manager,
    )
    dice_roll_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(x, y + yoffset * 7, 270, 150),
        text="Roll: ",
        manager=manager,
    )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == build_road_button:
                    ...
                    print_road_buttons(
                        verts_surface,
                        new_game.current_player.settlements,
                        vert_manager,
                        road_buttons,
                    )
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == build_settlement_button:
                    print_empty_verts(
                        verts_surface, empty_verts, vert_buttons, vert_manager
                    )
                ...
                for row in vert_buttons:
                    for button in row:
                        if event.ui_element == button:
                            print(row)
                            try:
                                new_game.place_settlement(
                                    row[0], new_game.current_player
                                )
                                # row[1].visible = False
                                vert_manager.clear_and_reset()
                                ...
                                pygame_gui.windows.UIMessageWindow(
                                    ...
                                    html_message="Not Enough Resources!",
                                    manager=manager,
                                )
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
...
def update_labels(
    ...
    victory_points_label.set_text(
        "Victory Points: " + str(new_game.current_player.victory_points)
    )
    brick_label.set_text(
        "Brick: " + str(new_game.current_player.resources.get(TileResource.Brick))
    )
    lumber_label.set_text(
        "Lumber: " + str(new_game.current_player.resources.get(TileResource.Lumber))
    )
    ore_label.set_text(
        "Ore: " + str(new_game.current_player.resources.get(TileResource.Ore))
    )
    grain_label.set_text(
        "Grain: " + str(new_game.current_player.resources.get(TileResource.Grain))
    )
    wool_label.set_text(
        "Wool: " + str(new_game.current_player.resources.get(TileResource.Wool))
    )
...
def draw_board(board, destination):
    ...
    for coord in board.tiles:
        ...
        if board.tiles[coord] != "Water":
            destination.blit(
                textureToVal(board.tiles[coord].resource),
                hexToPixel(75, coord.q, coord.r, 0),
            )
            pygame.draw.circle(
                destination,
                WHITE,
                (
                    hexToPixel(75, coord.q, coord.r, 70)[0],
                    hexToPixel(75, coord.q, coord.r, 70)[1],
                ),
                20,
            )
            pygame_gui.elements.UILabel(
                relative_rect=pygame.Rect(
                    hexToPixel(75, coord.q, coord.r, -5)[0],
                    hexToPixel(75, coord.q, coord.r, -5)[1],
                    150,
                    150,
                ),
                text=str(board.tiles[coord].returnNum()),
                object_id=pygame_gui.core.ObjectID(class_id="@rollNums"),
                manager=manager,
            )
            ...
            for settlement in player.settlements:
                if settlement.s == "N":
                    pygame.draw.circle(
                        surface,
                        player.color,
                        vertToPixel(75, settlement.q, settlement.r, x, y),
                        10,
                    )
                if settlement.s == "S":
                    pygame.draw.circle(
                        surface,
                        player.color,
                        vertToPixel(75, settlement.q, settlement.r, x, y + 75 * 2),
                        10,
                    )
            ...
            buttons.append(
                (
                    ...
                    pygame_gui.elements.UIButton(
                        relative_rect=pygame.Rect(
                            (vertToPixel(75, key.q, key.r, 50, -10)), (25, 25)
                        ),
                        text="",
                        object_id=pygame_gui.core.ObjectID(class_id="@vertButtons"),
                        manager=manager,
                    ),
                )
            )
        if key.s == "S":
            buttons.append(
                (
                    ...
                    pygame_gui.elements.UIButton(
                        relative_rect=pygame.Rect(
                            (vertToPixel(75, key.q, key.r, 50, -10 + 75 * 2)), (25, 25)
                        ),
                        text="",
                        object_id=pygame_gui.core.ObjectID(class_id="@vertButtons"),
                        manager=manager,
            ...
            if road.s == "W":
                road_buttons.append(
                    (
                        ...
                        pygame_gui.elements.UIButton(
                            relative_rect=pygame.Rect(
                                (vertToPixel(75, road.q, road.r, -20, 50)), (25, 25)
                            ),
                            text="",
                            object_id=pygame_gui.core.ObjectID(class_id="@roadButtons"),
                            manager=manager,
                        ),
                    )
                )
            if road.s == "NW":
                road_buttons.append(
                    (
                        ...
                        pygame_gui.elements.UIButton(
                            relative_rect=pygame.Rect(
                                (vertToPixel(75, road.q, road.r, 10, 5)), (25, 25)
                            ),
                            text="",
                            object_id=pygame_gui.core.ObjectID(class_id="@roadButtons"),
                            manager=manager,
            ...
            if road.s == "NE":
                road_buttons.append(
                    (
                        ...
                        pygame_gui.elements.UIButton(
                            relative_rect=pygame.Rect(
                                (vertToPixel(75, road.q, road.r, 90, 5)), (25, 25)
                            ),
                            text="",
                            object_id=pygame_gui.core.ObjectID(class_id="@roadButtons"),
                            manager=manager,
